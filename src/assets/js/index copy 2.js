 
const mercadopago = new MercadoPago("TEST-806f26a3-e8fc-4873-ad2a-38402d3505d5");
let obJectPay = new Object
var isReload = false;
let tokenV1;

function getToken(){
    return obJectPay
}


function loadCardForm() {
    const productCost = document.getElementById('amount').value;
    const productDescription = document.getElementById('product-description').innerText;


      cardForm = mercadopago.cardForm({
        amount: productCost,
        autoMount: true,
        form: {
            id: "form-checkout",
            cardholderName: {
                id: "form-checkout__cardholderName",
                placeholder: "Holder name",
            },
            cardholderEmail: {
                id: "form-checkout__cardholderEmail",
                placeholder: "E-mail",
            },
            cardNumber: {
                id: "form-checkout__cardNumber",
                placeholder: "Card number",
            },
            cardExpirationMonth: {
                id: "form-checkout__cardExpirationMonth",
                placeholder: "MM",
            },
            cardExpirationYear: {
                id: "form-checkout__cardExpirationYear",
                placeholder: "YY",
            },
            securityCode: {
                id: "form-checkout__securityCode",
                placeholder: "Security code",
            },
            installments: {
                id: "form-checkout__installments",
                placeholder: "Installments",
            },
            identificationType: {
                id: "form-checkout__identificationType",
            },
            identificationNumber: {
                id: "form-checkout__identificationNumber",
                placeholder: "Identification number",
            },
            issuer: {
                id: "form-checkout__issuer",
                placeholder: "Issuer",
            },
        },
        callbacks: {
            onFormMounted: error => {
                if (error){
                document.getElementById("loading-message").innerText = "Oops deu erro!";
                return console.warn("Form Mounted handling error: ", error);
                }
            },
            onSubmit: event => {
                event.preventDefault();
            if(isReload){
                atualizarToken();
                document.getElementById("loading-message").innerText = "...";
                isReload = false;
                load(false);
            }else{
                 load(true);
                 c = {
                    paymentMethodId,
                    issuerId,
                    cardholderEmail: email,
                    amount,
                    token,
                    installments,
                    identificationNumber,
                    identificationType,
                } = cardForm.getCardFormData();

                
                console.log('TOKE ' + toke)

                let mId = $("#form-checkout__issuer").val();

 }
            },
            onFetching: (resource) => {
                const payButton = document.getElementById("form-checkout__submit");
                payButton.setAttribute('disabled', true);
                return () => {
                    payButton.removeAttribute("disabled");
                };
            },
        },
    });
};

function load(isProcess){
if(isProcess){
document.getElementById("form-checkout__submit").style.backgroundColor = "#ffb42b";
document.getElementById("form-checkout__submit").innerText = "processando...";
document.getElementById("loading-message").innerText = "...";
var heightPage = document.body.scrollHeight;
window.scrollTo(0 , heightPage);

}
else{
document.getElementById("form-checkout__submit").style.backgroundColor = "#1ee9cd";
document.getElementById("form-checkout__submit").innerText = "Pagar";
}
}

function erroResend(mensagem){
    document.getElementById("loading-message").style.display = "block";
    document.getElementById("loading-message").innerText = mensagem;
    document.getElementById("form-checkout__submit").style.backgroundColor = "#ffb42b";
    document.getElementById("form-checkout__submit").innerText = "Refazer";
    isReload = true;
}

$( "#form-checkout__installments" ).change(function() {
 $("#total").val($("#form-checkout__installments").name());
});

function selectUpdate(card) {
 let unitPrice = document.getElementById('unit-price').value;

 const parcelas =  mercadopago.getInstallments({
  amount: unitPrice,
  locale: 'pt-BR',
  bin: card,
  processingMode: 'aggregator'
})
var x = Promise.resolve(parcelas);
var xx = Promise.resolve(x);
xx.then(function(v) {
$('#form-checkout__installments').empty();
for(var i = 0; i < v.length; i++){
for(var j = 0; j < v[i].payer_costs.length ; j++){
var instal = v[i].payer_costs[j];
var valor = instal.installment_amount;
var total = instal.total_amount;
var parcela = valor.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"});
var totalAmount = total.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"});

$('#form-checkout__installments').append(`<option name='${totalAmount}'
value='${instal.installments}'>${instal.installments + "X " + parcela + "\t\t(" + totalAmount + ")"}</option>`);
    }
}
});
}
function selectUpdateIssuer() {
 let card = $("#numeroCartao").val().replaceAll(" ", "").trim();
let result = card;
if(card.length > 5 && card.length < 7 || card.length > 14){
selectUpdate(card);
	 if(card.includes(" ", 0) == false){
 result = card.substring(0, 6);
 const paymentMethods =  mercadopago.getPaymentMethods({ bin: result });
var x = Promise.resolve(paymentMethods);
var xx = Promise.resolve(x);
xx.then(function(v) {
$('#form-checkout__issuer').empty();
$("#bandeira").attr("src", v.results[0].secure_thumbnail);
$('#form-checkout__issuer').append(`<option name='${v.results[0].issuer.id}'
value='${v.results[0].id}'>${v.results[0].issuer.name}</option>`);
obJectPay.issuerId = v.results[0].id;
obJectPay.paymentMethodId = v.results[0].issuer.id;
if(v.results[0].issuer.id == 18){
$("#form-checkout__securityCode").attr("pattern", "[0-9]{4}");
}
else{
$("#form-checkout__securityCode").attr("pattern", "[0-9]{3}");
}
});
}
}
}
function atualizarToken(){
let cartao = $("#form-checkout__cardNumber").val().replaceAll(" ", "");
const card =  mercadopago.createCardToken({
       cardNumber: cartao,
       cardholderName: $("#form-checkout__cardholderName").val(),
       cardExpirationMonth: $("#form-checkout__cardExpirationMonth").val(),
       cardExpirationYear: $("#form-checkout__cardExpirationYear").val(),
       securityCode: $("#form-checkout__securityCode").val(),
       identificationType: 'CPF',
       identificationNumber: $("#form-checkout__identificationType").val(),
    });
     var original = Promise.resolve(card);
     var cast = Promise.resolve(original);
     cast.then(function(v) {
     $("#MPHiddenInputToken").val(v.id); 
     
     obJectPay.cardholderEmail = $("#form-checkout__cardholderEmail").val()
     obJectPay.amount = $("#unit-price").val()
     obJectPay.token = v.id
     obJectPay.installments = $("#form-checkout__installments").val()
     obJectPay.identificationNumber = $("#form-checkout__identificationNumber").val()
     obJectPay.identificationType = $("#form-checkout__identificationType").val()   

    });
    selectUpdateIssuer();

}

// Handle transitions
function showCheckout(){
    $('.container__cart').fadeOut(500);
    setTimeout(() => {
        loadCardForm();
        $('.container__payment').show(500).fadeIn();
    }, 500);
};

  function showCart(){
    $('.container__payment').fadeOut(500);
    setTimeout(() => { $('.container__cart').show(500).fadeIn(); }, 500);
};





