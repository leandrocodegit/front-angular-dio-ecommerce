...
value='${v.results[0].id}'>${v.results[0].issuer.name}</option>`);
                obJectPay.issuerId = v.results[0].issuer.id;
                obJectPay.paymentMethodId = v.results[0].id;
...
function atualizarToken() {
    ...
    if (getValid()) {
        ...
        cast.then(function (v) {
            ...
            obJectPay.transactionAmount = $("#unit-price").val()
            obJectPay.installments = $("#form-checkout__installments").val()
            obJectPay.productDescription = $('#product-description').text();
